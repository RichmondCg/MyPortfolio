import { useEffect, useRef } from "react";
import gsap from "gsap";
import { Link } from "react-router-dom";
import Navigation from "./Navigation.jsx";

const EXPERIENCE = [
  {
    role: "Web Developer",
    place: "Freelance",
    period: "2026 — Present",
    desc: "Building full-stack web systems and websites for clients — combining solid engineering with a strong focus on UI/UX, while also handling SEO and performance optimization.",
  },
  {
    role: "Software Developer (OJT)",
    place: "Metaverse Holdings Corporation",
    period: "Jan 2026 — May 2026",
    desc: "Developed landing pages and frontend components for an appointment system.",
  },
  {
    role: "Data Entry Associate",
    place: "Dagaz HR Consultancy and Recruitment Company",
    period: "Mar 2025 — Sept 2025",
    desc: "Maintained 100% data entry accuracy while applying good judgment in a fast-paced environment.",
  },
  {
    role: "IoT Developer",
    place: "Freelance",
    period: "2023 — 2025",
    desc: "Built automated systems using Arduino and ESP8266 microcontrollers, with SMS functionality and mobile application integration.",
  },
];

function Experience() {
  const curtainRef = useRef(null);
  const headRef = useRef(null);
  const introRef = useRef(null);
  const listRef = useRef(null);
  const footRef = useRef(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      const tl = gsap.timeline();

      tl.set([headRef.current, introRef.current], { autoAlpha: 0, y: 30 })
        .set([listRef.current, footRef.current], { autoAlpha: 0, y: 40 })
        .to(
          curtainRef.current,
          { yPercent: -100, duration: 0.7, ease: "power4.inOut" },
          0,
        )
        .to(
          [headRef.current, introRef.current],
          { autoAlpha: 1, y: 0, duration: 0.9, stagger: 0.1 },
          "-=0.25",
        )
        .to(
          listRef.current,
          { autoAlpha: 1, y: 0, duration: 0.8, ease: "power3.out" },
          "-=0.4",
        )
        .to(footRef.current, { autoAlpha: 1, y: 0, duration: 0.6 }, "-=0.3");
    });

    return () => ctx.revert();
  }, []);

  return (
    <div className="relative min-h-screen w-full bg-white font-mono text-black select-none">
      <div
        ref={curtainRef}
        className="absolute inset-0 z-50 bg-white will-change-transform"
      />

      <Navigation />

      <main className="mx-auto max-w-5xl px-6 pt-32 md:px-12 md:pt-44">
        <p className="mb-4 text-[11px] tracking-[0.32em] uppercase opacity-60">
          01 · Experience
        </p>
        <h1
          ref={headRef}
          className="font-display text-[clamp(2.5rem,8vw,7rem)] leading-none tracking-tight"
        >
          Experience
        </h1>

        <div ref={introRef} className="mt-8 max-w-2xl">
          <p className="text-sm leading-relaxed md:text-base">
            A journey of roles, projects, and skills — from the first line of
            code to building full-stack products with a design-first mindset.
          </p>
        </div>

        <div ref={listRef} className="mt-16 space-y-10">
          {EXPERIENCE.map((exp) => (
            <div
              key={exp.role}
              className="grid gap-2 md:grid-cols-[160px_1fr] md:gap-8"
            >
              <p className="text-xs tracking-[0.2em] uppercase text-black/50 md:pt-1">
                {exp.period}
              </p>
              <div>
                <h3 className="font-display text-lg md:text-xl">{exp.role}</h3>
                <p className="text-xs tracking-[0.2em] uppercase text-black/50">
                  {exp.place}
                </p>
                <p className="mt-2 max-w-xl text-sm leading-relaxed text-black/80">
                  {exp.desc}
                </p>
              </div>
            </div>
          ))}
        </div>

        <div ref={footRef} className="my-10 border-t border-black/10 py-10">
          <Link
            to="/me"
            className="group inline-flex items-center gap-4 rounded-full border border-black px-8 py-4 text-[11px] tracking-[0.25em] uppercase transition-colors duration-300 hover:bg-black hover:text-white"
          >
            Back to about
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              aria-hidden="true"
              className="transition-transform duration-300 group-hover:-translate-x-1 group-hover:translate-y-1"
            >
              <path
                d="M11 1L1 11M1 11H9M1 11V3"
                stroke="currentColor"
                strokeWidth="1.5"
              />
            </svg>
          </Link>
        </div>
      </main>
    </div>
  );
}

export default Experience;
